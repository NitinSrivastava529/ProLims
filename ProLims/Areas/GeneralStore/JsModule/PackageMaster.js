...
function Onload() {
    var url = config.baseUrl + "/api/GeneralStore/PackageQueries";
    var objBO = {};
    objBO.Logic = "OnLoad";
    $.ajax({
        method: "POST",
        ...
        success: function (data) {
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                ...
                if (Object.keys(data.ResultSet.Table1).length > 0) {
                    ...
                    $('#ddlSubCategory').empty().append($('<option></option>').val('0').html('Select'));
...
function InsertUpdateLabPackage() {
    if (validation()) {
        var objBO = {};
        var url = config.baseUrl + "/api/GeneralStore/mPackageInsertUpdate";
...
function LoadPackage() {
    ...
    //    objBO.Logic = "LoadPackageItmetype";
    //}
    //else {
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table).length > 0) {
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        htmldata += '<td style="text-align:center"><a href="javascript:void(0)" id="btnSelect' + key + '" data-itemid="' + val.ItemId + '"  onclick=selectRow(this);SelectPackage(this.id,"")><i class="fa fa-check fa-lg text-red" ></i></a></td>';
                        htmldata += '</tr>';
                    });
        ...
        success: function (data) {
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table).length > 0) {
                    ...
                    $("#ddlSubCategory").val(data.ResultSet.Table[0].SubCategoryID).prop('selected', true);
                    ...
                    $("#txtrate").val(data.ResultSet.Table[0].rate)
            ...
            if (Object.keys(data.ResultSet).length > 0) {
                if (Object.keys(data.ResultSet.Table).length > 0) {
                    $.each(data.ResultSet.Table, function (key, val) {
                        ...
                        htmldata += '<td>' + val.ItemId + '</td>';
                        htmldata += '<td>' + val.ItemName + '</td>';
                        htmldata += '<td style="text-align:center"><a href="javascript:void(0)" id="btnLinkPackInvest' + key + '" data-sourceitemid="' + val.SourceItemId + '" data-itemid="' + val.ItemId + '"  data-searchtype="' + val.searchType + '" onclick="selectRow(this);LinkPackageInvestigation(this.id)"><i class="fa fa-plus fa-lg text-green"></i></a></td>';
                        htmldata += '</tr>';
                    });
...
function LinkPackageInvestigation(buttonid) {
    debugger
    var objBO = {};
    var url = config.baseUrl + "/api/GeneralStore/mPackageLinkItems";
