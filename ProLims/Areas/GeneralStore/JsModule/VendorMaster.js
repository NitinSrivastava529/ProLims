...
$(document).ready(function () {
    $('#ddlState').empty().append($('<option>Select State</option>')).select2();
    GetVendorMasterList();
        ...
        success: function (data) {          
            if (data != '') {
                ...
                $.each(data.ResultSet.Table3, function (key, val) {
                    $('<tr><td>' +
                        '<category data-vendorid="' + val.vendor_id + '" class="btn btn-warning btn-sm getVendor"><i class="fa fa-edit"></i></category></td>' +
                        '<td>' + val.vendor_id + '</td>' +
                        '<td>' + val.vendor_name + '</td>' +
                        '<td class="text-center">' +
                        "<div data-init='mm' class='toggle mb-3 " +
                        (val.active_flag === 'Y' ? 'on' : 'off') +
                        "' data-venderid='" + val.vendor_id + "' data-flag='" + val.active_flag + "' onclick='Toggle(this)'><span></span></div>" +
                        '</td></tr>').appendTo($('#tblVendor tbody'));
                });
...
function Toggle(elem) {
    var venderid = $(elem).data('venderid');
    $(elem).toggleClass('on')
    UpdateStatus(venderid)
}
...
function GetStateByCountry(cId) {
    var url = config.baseUrl + "/api/GeneralStore/MasterQueries";
    var objBO = {};
    objBO.country_id = $('#ddlCountry option:selected').val();
...
function InsertVendorMaster() {
    if (Validate()) {
        var url = config.baseUrl + "/api/GeneralStore/InsertUpdateVendorMaster";
        var objBO = {};
...
function UpdateStatus(vendorid) {
    var url = config.baseUrl + "/api/GeneralStore/InsertUpdateVendorMaster";
    var objBO = {};
    objBO.vendor_id = vendorid;
    objBO.active_flag = '-';
