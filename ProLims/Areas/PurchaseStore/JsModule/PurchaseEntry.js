...
$(document).ready(function () {
    ...
    $('#tblTaxEntry tbody').on('click', '#deleteTax', function () {
        var autoId = $(this).data('autoid');
        var itemid = $(this).data('itemid');
        deleteTax(autoId, itemid);
    });
});
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    $('#ddlPackType').append($('<option data-text="' + val.pack_type + '" data-pqty=' + val.pack_qty + '></option>').val(val.pack_id).html(val.pack_type + '/' + val.pack_qty));
                });
            }
            else {
                alert("Error");
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    $("#ddlManufacture").append($("<option data-mfdid=" + val.mfd_id + "></option>").val(val.mfd_id).html(val.mfd_name));
                });
            }
            else {
                alert("Error");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    $('#tblnavigate').show();
                    $('<tr class="searchitems" data-hsn=' + val.hsn + ' data-itemtype=' + val.item_type + '  data-itemid="' + val.item_id + '"><td>' + val.item_name + "</td></tr>").appendTo($('#tblnavigate tbody'));
                });
            }
            else {
                alert("Error");
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    $('#ddlGst').append($('<option data-taxid=' + val.tax_id + '></option>').val(val.tax_rate).html(val.tax_details));
                });
            }
            else {
                alert("Error");
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    ...
                    str += "<td style='text-align:center'>" + val.MRP + "</td>";
                    str += "<td style='text-align:center'>" + val.trade + "</td>";
                    str += "<td style='text-align:center'>" + val.npr + "</td>";
                    str += "<td style='text-align:center'>" + val.Quantity + "</td>";
                    str += "<td style='text-align:center'>" + val.It_Free + "</td>";
                    str += "<td style='text-align:center'>" + val.tax_rate + "</td>";
                    str += "<td style='text-align:center'>" + val.tax_amount + "</td>";
                    str += "<td style='text-align:center'>" + val.amount + "</td>";
                    str += "<td style='text-align:center'>" + val.DisPer + "</td>";
                    str += "<td style='text-align:center'>" + val.DisAmount.toFixed(2) + "</td>";
                ...
                $.each(data.ResultSet.Table3, function (key, val) {
                    $('#txtTotal').val(val.TaxableAmount);
                    $('#txtDisTotal').val(val.TaxableAmount);
                    //$('#txtDiscount').val(val.discount);
                    $('#txtIGST').val(val.IGST);
                    $('#txtSGST').val(val.SGST);
                    $('#txtCGST').val(val.CGST);
                    $('#txtNetAmount').val(val.TotalValue);
                    $('#txtRoundOff').val(val.TotalValue);
                });
            }
            else {
                alert("Error");
...
function InsertPurchase() {
    if (validation()) {
        var url = config.baseUrl + "/api/GeneralStore/PurchaseInsert";
        var objBO = {};
        objBO.CompId = Active.compId;
        objBO.UnitId = Active.unitId;
        objBO.Grn_type = $('#ddlGrnType option:selected').val();
        ...
        objBO.Tax_id = $('#ddlGst option:selected').data('taxid');
        objBO.Remark = '';
        objBO.CreatedBy = Active.userId;
        objBO.PayMode = $('input[name=PayMode]:checked').val();
        objBO.GrnNo = $('#txtPurchaseId').val();
        $.ajax({
            method: "POST",
            url: url,
            data: JSON.stringify(objBO),
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table1, function (key, val) {
                    ...
                    $('input[name=PayMode][value="' + val.Paymode + '"]').prop('checked', true);
                    otherchargesList();
                });
                $('#BasicInfo').find('.form-control').attr('disabled', true).css({ 'background': '#fff', 'opacity': '100%' });
                $('#tblPaymentDetails').find('.form-control').css({ 'background': '#fff', 'opacity': '100%' });
    ...
    $.ajax({
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    $('<tr><td>' + val.Purch_id + '</td><td>' + val.Inv_No + '</td><td>' + val.Inv_Date + '</td><td>' + val.vendor_name + '</td><td><button class="btn btn-success getUnCompPurchase" style="height:25px" data-pId=' + val.Purch_id + '><i class="fa fa-check"></i>&nbsp;Select</button></td></tr>').appendTo($('#tblUnCompPurchase tbody'));
                });
            }
            else {
                alert("Error");
...
function DeletePurchase(autoId, purchaseId) {
    if (confirm('Are you sure want to delete this Purchase Entry..')) {
        var url = config.baseUrl + "/api/GeneralStore/DeleteGRN";
        var objBO = {};
...
function DiscountReprocess() {
    if (disValidation()) {
        var url = config.baseUrl + "/api/GeneralStore/GRNDiscountReprocess";
        var objBO = {};
...
function GetOtherChargeList() {
    var url = config.baseUrl + "/api/GeneralStore/PurchaseQueries";
    var objBO = {};
    objBO.Logic = 'GetOtherChargesName';
    $.ajax({
        method: "POST",
        url: url,
        data: JSON.stringify(objBO),
        ...
        success: function (data) {
            if (data != '') {
                $('#ddlcharges').empty().append($('<option value="Select">Select</option>'));
                $.each(data.ResultSet.Table, function (key, val) {
                    $('#ddlcharges').append($('<option></option>').val(val.item_id).html(val.item_name));
                });
            }
            else {
                alert("Error");
...
function insertTax() {
    if ($('#ddlcharges option:selected').val() == 'Select') {
        alert('Please select Other Charges');
        return;
    }
    var url = config.baseUrl + "/api/GeneralStore/GRNOtherCharges";
    var objBO = {};
    objBO.Autoid = '0';
    objBO.GrnNo = $('#txtPurchaseId').val();
    objBO.Item_id = $('#ddlcharges option:selected').val();
    objBO.TaxableAmount = $('#txtTaxableAmt').val();
    objBO.taxRate = $('#txtTaxableRate').val();
    objBO.Tax = $('#txtTaxvalue').val();
    objBO.vendor_code = $('#ddlSupplier option:selected').val();
    objBO.login_id = Active.userId;
    objBO.Logic = 'OtherCharges:Insert';
    $.ajax({
        method: "POST",
        url: url,
        data: JSON.stringify(objBO),
...
function clearTax() {
    $('#txtTaxableAmt').val('0');
    $('#txtTaxableRate').val('0');
    $('#txtTaxvalue').val('0');
    $('#ddlcharges').val('select').prop('selectedIndex', '0').change();
}
function PurchasePosting() {
    var url = config.baseUrl + "/api/Canteen/pCant_PurchasePosting";
    var objBO = {};
    objBO.Purch_id = $('#txtPurchaseId').val();
    objBO.adj_amt = 0;
    objBO.plusminus = 0;
...
function otherchargesList() {
    $("#tblTaxEntry tbody").empty();
    var url = config.baseUrl + "/api/GeneralStore/PurchaseQueries";
    var objBO = {};
    objBO.purchId = $('#txtPurchaseId').val();
        ...
        success: function (data) {
            if (data != '') {
                ...
                $.each(data.ResultSet.Table, function (key, val) {
                    ...
                    tbody += "<td style='text-align:center'>" + val.TaxableAmount + "</td>";
                    tbody += "<td style='text-align:center'>" + val.TaxableRate + "</td>";
                    tbody += "<td style='text-align:center'>" + val.Tax + "</td>";
                    tbody += "<td style='text-align:center'>" +
                        '<button type="button" id="deleteTax" data-autoid="' + val.AutoId + '"  data-itemid="' + val.ItemId + '" class="btn btn-danger btn-xs"> <i class="fa fa-close"></i></button> ' +
                        "</td>";
                    tbody += "</tr>";
                });
                $("#tblTaxEntry tbody").append(tbody);
...
function deleteTax(autoId, itemid) {
    var url = config.baseUrl + "/api/GeneralStore/GRNOtherCharges";
    var objBO = {};
    objBO.Autoid = autoId;
    objBO.GrnNo = $('#txtPurchaseId').val();
    objBO.Item_id = itemid;
    objBO.TaxableAmount = '0';
    objBO.taxRate = '0';
    objBO.Tax = '0';
    objBO.vendor_code = '-';
    objBO.login_id = Active.userId;
    objBO.Logic = 'OtherCharges:Delete';
    $.ajax({
        method: "POST",
        url: url,
        data: JSON.stringify(objBO),
