...
function CreateChandanCareCard() {
    if (Validate1()) {
        ...
        var url = config.baseUrl + "/api/Patient/CreateChandanCareCard";
        var obj = {};
        obj.UnitId = 'CH01';
...
function browseFile(event) {
    if (event.files && event.files[0]) {
        ext = $('input[id=fileUpload]').val().split('.').pop().toLowerCase();
        ...
        var kb = ((event.files[0].size) / 1024).toFixed(2);
        var mb = ((event.files[0].size) / 1048576).toFixed(2);
        var size = (kb < 1024) ? (kb + ' KB') : (mb + ' MB');
    }

    var item = "";
    item += '<div class="fileItem">';
    item += '<div class="left">';
    item += '<div class="cancel fa fa-remove" onclick=$(this).closest(".fileItem").remove()></div>';
    item += '<div class="file">';
    item += '<label class="name">' + filename + '</label>';
    item += '<label class="size">' + size + '</label>';
    item += '<label class="vpath hide"></label>';
    item += '</div>';
    item += '</div>';
    item += '<div class="right">';
    item += '<div class="loader"><span>00</span></div>';
    item += '</div>';
    item += '</div>';

    $('.attach').append(item);
    $('.attach .fileItem:last').find('div.right').show();
    UploadDocument();
}
function UploadDocument() {
    var url = config.baseUrl + "/api/Patient/UploadB2BDocument";
    var UploadDocumentInfo = new XMLHttpRequest();
    var data = new FormData();
    data.append('extention', $('input[id=fileUpload]').val().split('.').pop());
    data.append('ImageByte', $('input[id=fileUpload]')[0].files[0]);
    UploadDocumentInfo.upload.addEventListener("progress", function (evt) {
        if (evt.lengthComputable) {
            var percentComplete = evt.loaded / evt.total;
            percentComplete = parseInt(percentComplete * 100);            
            if (percentComplete === 100) {
                $('.attach .fileItem:last').find('div.right').hide();
            }
        }
    }, false);
    UploadDocumentInfo.onreadystatechange = function () {
        if (UploadDocumentInfo.status) {
            if (UploadDocumentInfo.status == 200 && (UploadDocumentInfo.readyState == 4)) {
                var vpath = JSON.parse(UploadDocumentInfo.responseText);
                console.log(vpath)
                $('.attach .fileItem:last').find('div.file .vpath').text(vpath);
            }
        }
    }
